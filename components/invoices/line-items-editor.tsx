...
export function LineItemsEditor({ items, currency = "USD", onChange }: LineItemsEditorProps) {
  ...
  return (
    <div className="w-full rounded-lg border border-border overflow-hidden">
      {/* Table header */}
      <div className="grid grid-cols-[28px_1fr_80px_130px_110px_40px] gap-0 bg-muted/60 border-b border-border">
        ...
        {items.map((item, idx) => {
          ...
          return (
            <div
              key={item.id}
              className={cn(
                "grid grid-cols-[28px_1fr_80px_130px_110px_40px] gap-0 items-center group transition-colors",
                "hover:bg-accent/40"
              )}
            >
              {/* Row number */}
              ...
              <div className="py-1.5 pr-2">
                <Input
                  ...
                  className="h-8 text-sm border bg-transparent shadow-none focus-visible:border-input focus-visible:bg-background focus-visible:shadow-xs"
                />
              </div>
